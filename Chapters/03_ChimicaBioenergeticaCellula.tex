...
\ce{H_2O}. Questo processo \`e chiamato respirazione aerobica. La fofosintesi e la respirazione sono processi complementari. Si nota pertanto come l'utilizzo di carbonio formi un grande 
...
Questa reazione \`e accoppiata con molte altre di sintetizzazione. Una tipica reazione biosintetica \`e una in cui due molecole $A$ e $B$ sono unite per produrre \ce{A-B} durante la 
condensazione: 
$$\ce{A-H + B-OH -> A-B + H2O}$$
...
parte di enzimi. Molte di queste parti contengono un nucleotide (solitamente adenosima). Si noti come \ce{ATP} trasferisce fosfato, \ce{NADPH} elettroni e idrogeno. Altri vettori 
attivati traferiscono gruppi utilizzati per la biosintesi e sono generati in reazioni accoppiati con l'idrolisi dell'\ce{ATP}.
...
cellula, dove avviene una graduale ossidazione. Gli zuccheri sono un'importante fonte di energia e vengono ossidati in passi ontorllati in anidride carbonica (\ce{CO2}) e acqua. 
...
idrolizzate due molecole di \ce{ATP} per fornire energia nei primi passaggi, ma alla fine sono prodotte quattro molecole di \ce{ATP}, con un guagagno netto di due \ce{ATP} e di 
...
rimuovendo elettroni grazie a \ce{NAD+} producendo \ce{NADH} dal carbonio derivato dalla molecola di glucoso. La natura a passaggi rilascia l'energia in piccoli pacchetti che possono 
...
di \ce{FADH2} (flavina adenina dinucleotide ridotto) viene prodotta da \ce{FAD} e il ribonucleoside trifosfato \ce{GTP} dal \ce{GDP}, molto simile dell'\ce{ATP}, il cui trasferimento 
...
\subsection{Il trasporto di elettroni guida la sintesi della maggori parte dell'\ce{ATP} nelle cellule}
La maggior parte dell'energia chimira rilasciata nell'ultima perte della degradazione di un nutriente permette il trasferimento da parte di \ce{NADH} e \ce{FADH2} degli elettroni che
...
mitocondriale allo spazio intermembrana e al citosol, generando un gradiente di ioni \ce{H+} che serve come fonte di energia per la cellula per reaizoni come la generazione di \ce{ATP}
attraverso fosforilazione di \ce{ADP}. ALla fine della serie di trasferimenti gli elettroni sono passati a molecole di ossigeno gassoso diffuso nel mitocondrio che producono acqia. 
...
metabolica. Carbonio e idorgeno formano \ce{CO2} e \ce{H2O}, mentre gli atomi di azoto sono spostati in varie forme e appaiono come urea, che viene secreta. Ogni amminoacido viene
...
metioneina, cisteina, del coenzima A e i centri a ferro-zolfo per il traporto di elettroni. La riduzione dello zolfo comincia in batteri, funghi e piante, dove enzimi e \ce{ATP} sono 
